...
Aluguel* InsereAluguel(Aluguel* lista, Veiculo* veiculo, Cliente* cliente, int cod, const char* cidade) {
	...
	if (novo == NULL) {
		printf("Erro ao alocar memória para o novo aluguel.\n");
		return lista;
	}

	// Define os valores do novo aluguel
	novo->cod = cod;
	...
	if (lista == NULL) {
		return novo; // O novo aluguel será o primeiro da lista
	}
	...
	// Insere o novo aluguel no final da lista
	atual->next = novo;
...
bool GravarAluguelBin(char* nomeFicheiro, Aluguel* inicio) {
	...
	while (auxAluguel != NULL) {
		AluguelFicheiro aluguelFicheiro;
		aluguelFicheiro.cod = auxAluguel->cod;
		strcpy(aluguelFicheiro.cidade, auxAluguel->cidade);
